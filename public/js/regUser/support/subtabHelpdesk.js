(function () {
	...
	function getInitials(name, fallback = 'SP') {
	...
	function mapContactToTarget(contact, targetType) {
		...
		return {
			...
			targetUserId: id,
			name,
			subtitle: targetType === 'Driver' ? 'Driver support contact' : 'Guide support contact',
			avatar: getInitials(name, targetType === 'Driver' ? 'DR' : 'GD')
		};
	}
	...
	function renderTargetList() {
		...
		const filtered = state.targets.filter((target) => {
			...
			if (filter === 'drivers') return target.targetType === 'Driver';
			if (filter === 'guides') return target.targetType === 'Guide';
			return true;
		});
	...
	function renderWelcomeState() {
		elements.messages.innerHTML = `
			<div class="empty-chat-state">
				...
				<p>Choose Site Support, a driver, or a guide to start messaging.</p>
			</div>
		`;
	}
	...
	function isOutgoingMessage(message) {
		...
		return String(message.sender_type || '') === String(state.viewerSenderType || 'Traveller');
	}
	...
	async function loadTargets() {
		...
		try {
			const result = await requestJson(`${URL_ROOT}/helpc/getTravellerSupportContacts`);
			...
			drivers = (data.drivers || []).map((contact) => mapContactToTarget(contact, 'Driver'));
			guides = (data.guides || []).map((contact) => mapContactToTarget(contact, 'Guide'));
		} catch (error) {
			showNotification('Could not load provider contacts. Site support is still available.', 'warning');
		}

		state.targets = [siteTarget, ...drivers, ...guides];
		renderTargetList();
