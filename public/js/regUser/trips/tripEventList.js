(function() { 
    ...
    class TripEventListManager{
        ...
        openDriverSelection(segmentIndex = 0) {
            ...
            // Check if a driver is already selected for this segment
            const currentDriver = this.selectedDrivers && this.selectedDrivers[segmentIndex];
            
            if (currentDriver) {
                // If driver exists, check their request status
                if (currentDriver.requestStatus === 'rejected') {
                    // Allow selection of new driver
                    console.log(`Driver rejected for segment ${segmentIndex}, allowing new selection`);
                } else {
                    // Driver is in pending/accepted status, prevent selection
                    const statusText = currentDriver.requestStatus === 'pending' ? 'Pending Response' : 
                                     (currentDriver.requestStatus === 'accepted' ? 'Accepted' : currentDriver.requestStatus);
                    alert(`Driver request is currently ${statusText}. You can only select a new driver if the current request is rejected.`);
                    return;
                }
            }
        ...
        async saveGuideRequest(eventId, travelSpotId) {
            ...
            try {
                ...
                if (this.selectedGuide) {
                    ...
                    const baseCharge = this.selectedGuide.convertedCharge || this.selectedGuide.baseCharge || 0;
                    const chargeType = this.selectedGuide.chargeType || 'whole_trip';
                    const numberOfPeople = this.numberOfPeople || 1;
                    const totalCharge = (chargeType === 'per_person') ? baseCharge * numberOfPeople : baseCharge;
                    ...
                    guideRequestData.chargeType = chargeType;
                    guideRequestData.numberOfPeople = numberOfPeople;
                    guideRequestData.totalCharge = totalCharge;
        ...
        async loadExistingDrivers() {
            try {
                ...
                if (result.success && result.drivers && Object.keys(result.drivers).length > 0) {
                    ...
                    // Update button to show pending state since drivers are already saved
                    // But only if not all drivers are rejected
                    const allRejected = Object.values(this.selectedDrivers).every(d => d.requestStatus === 'rejected');
                    
                    if (!allRejected) {
                        this.confirmTripBtn.textContent = 'Pending';
                        this.confirmTripBtn.disabled = true;
                        this.confirmTripBtn.style.backgroundColor = '#ffa500'; // Orange color for pending
                    }
                } else {
        ...
        async confirmTrip() {
            ...
            try {
                ...
                if (result.success) {
                    ...
                    // Change button text to "Pending" if drivers were saved
                    if (Object.keys(this.selectedDrivers).length > 0) {
                        this.confirmTripBtn.textContent = 'Pending';
                        this.confirmTripBtn.disabled = true;
                        this.confirmTripBtn.style.backgroundColor = '#ffa500'; // Orange color for pending
                    }
                                                    ...
                                                    ${driverInfo ? `
                                                        <div class="driver-selected">
                                                            ...
                                                            <div class="driver-info">
                                                                ...
                                                                ${driverData && driverData.requestStatus ? `<div class="driver-status-badge status-${driverData.requestStatus}">${driverData.requestStatus === 'pending' ? '⏳ Pending' : driverData.requestStatus === 'accepted' ? '✅ Accepted' : '❌ Rejected'}</div>` : ''}
                                                            </div>
                                                            <button class="change-driver-btn" onclick="window.tripEventListManager.openDriverSelection(${index})" title="${driverData && driverData.requestStatus === 'rejected' ? 'Select Another Driver' : 'Driver request in progress'}" ${driverData && driverData.requestStatus !== 'rejected' ? 'disabled style="opacity: 0.5; cursor: not-allowed;"' : ''}>
                                                                <i class="fas fa-exchange-alt"></i>
                                                            </button>
                                                        </div>
                                                    ` : `
