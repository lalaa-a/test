(function() {
    ...
    if (window.GuideRequestsManager) {
        console.log('GuideRequestsManager already exists, cleaning up...');
        if (window.guideRequestsManager) {
            delete window.guideRequestsManager;
        }
        delete window.GuideRequestsManager;
    }

    // Guide Requests Manager
    class GuideRequestsManager {
        constructor() {
        ...
        async loadRequests() {
            try {
                console.log('Loading guide requests...');
                const response = await fetch(`${this.URL_ROOT}/Guide/getMyRequests`);
        ...
        filterRequests(section, searchTerm = '') {
            ...
            if (searchTerm) {
                ...
                filteredRequests = filteredRequests.filter(req =>
                    req.userFullName.toLowerCase().includes(term) ||
                    req.totalCharge.toString().includes(term)
                );
            }
        ...
        createRequestRow(request, section) {
            const tripDate = new Date(request.createdAt).toLocaleDateString();
            ...
            if (section === 'pending') {
                dateValue = new Date(request.requestedAt || request.createdAt).toLocaleDateString();
            }
            ...
            if (section === 'pending') {
                actionButtons = `
                    <div class="request-actions">
                        <button class="btn btn-view" data-request-id="${request.id}">
                ...
                <tr>
                    <td>
                        <div class="traveller-info">
                            <img src="${this.UP_ROOT}${request.userProfilePhoto || 'profile/default.jpg'}" alt="Tourist" class="traveller-avatar">
                            <div>
                                <div class="traveller-name">${request.userFullName}</div>
                                <div class="traveller-rating">
                                    <i class="fas fa-star"></i> ${request.userAverageRating || 'N/A'}
                                </div>
                            </div>
                        </div>
                    </td>
                    <td>${tripDate}</td>
                    <td>
                        <span class="charge-badge">${request.chargeType === 'per_person' ? 'Per Person' : 'Whole Trip'}</span>
                    </td>
                    <td>
                        <span class="amount-display">${request.totalCharge} LKR</span>
                    </td>
                    <td>
                        <span class="people-badge">${request.numberOfPeople}</span>
                    </td>
                    <td>${dateValue}</td>
                    <td>${actionButtons}</td>
                </tr>
        ...
        showRequestDetails(requestId) {
            ...
            for (let section of Object.keys(this.requests)) {
                request = this.requests[section].find(r => r.id == requestId);
                if (request) break;
            }
            ...
            const tripDate = new Date(request.createdAt).toLocaleDateString();
            const requestedDate = new Date(request.requestedAt || request.createdAt).toLocaleDateString();
            const acceptedDate = request.respondedAt ? new Date(request.respondedAt).toLocaleDateString() : 'N/A';

            const content = `
                <div class="request-details-grid">
                    <div class="detail-group">
                        <label class="detail-label">Tourist Name</label>
                        <div class="detail-value">${request.userFullName}</div>
                    </div>
                    <div class="detail-group">
                        <label class="detail-label">Tourist Rating</label>
                        <div class="detail-value">
                            <i class="fas fa-star" style="color: #fbbf24;"></i> ${request.userAverageRating || 'No rating'}
                        </div>
                    </div>
                    <div class="detail-group">
                        <label class="detail-label">Travel Spot</label>
                        <div class="detail-value">${request.spotName || 'N/A'}</div>
                    </div>
                    <div class="detail-group">
                        <label class="detail-label">Number of People</label>
                        <div class="detail-value">${request.numberOfPeople}</div>
                    </div>
                    <div class="detail-group">
                        <label class="detail-label">Charge Type</label>
                        <div class="detail-value">${request.chargeType === 'per_person' ? 'Per Person' : 'Whole Trip'}</div>
                    </div>
                    <div class="detail-group">
                        <label class="detail-label">Total Amount</label>
                        <div class="detail-value highlighted">${request.totalCharge} LKR</div>
                    </div>
                    <div class="detail-group">
                    ...
                    <div class="detail-group">
                        <label class="detail-label">Guide Bio</label>
                        <div class="detail-value">${request.guideBio || 'No bio available'}</div>
                    </div>
                </div>
                ...
                <div class="request-summary">
                    ...
                    <div class="summary-row">
                        ...
                        <span>${request.totalCharge} LKR</span>
                    </div>
                </div>

                ${request.userProfilePhoto ? `
                    <div class="detail-group">
                        <label class="detail-label">Tourist Photo</label>
                        <img src="${this.UP_ROOT}${request.userProfilePhoto}" alt="Tourist" style="max-width: 100%; border-radius: 6px; cursor: pointer; max-height: 300px;" onclick="guideRequestsManager.viewPhoto('${this.UP_ROOT}${request.userProfilePhoto}')">
                    </div>
                ` : ''}
            `;
        ...
        async confirmAcceptRequest() {
            try {
                const response = await fetch(`${this.URL_ROOT}/Guide/acceptRequest/${this.pendingAcceptRequestId}`, {
                    method: 'POST',
                    headers: {
        ...
        async confirmRejectRequest() {
            ...
            try {
                const response = await fetch(`${this.URL_ROOT}/Guide/rejectRequest/${this.pendingRejectRequestId}`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json'
                    },
        ...
        switchModalTab(tabName) {
            ...
            const activeBtn = document.querySelector(`[onclick="guideRequestsManager.switchModalTab('${tabName}')"]`);
            if (activeBtn) {
        ...
        async loadItinerary() {
            ...
            let request = null;
            for (let section of Object.keys(this.requests)) {
                request = this.requests[section].find(r => r.id == this.currentRequestId);
                if (request) break;
            }
            ...
            try {
                const response = await fetch(`${this.URL_ROOT}/Guide/getTripItinerary/${request.tripId}`);
                const data = await response.json();
            ...
            sortedDates.forEach((date, index) => {
                ...
                html += `
                    <div class="summary-day-section">
                        <div class="summary-day-header" onclick="guideRequestsManager.toggleDaySummary(${index})">
        ...
        renderDayEvents(events) {
            return events.map(event => {
                ...
                const typeIcon = event.eventType === 'travelSpot' ?
                    'fa-map-marked-alt' : 'fa-map-marker-alt';
                const statusBadge = {
                    'start': 'Start Point',
    ...
    window.GuideRequestsManager = GuideRequestsManager;
    window.guideRequestsManager = new GuideRequestsManager();
