(function(){
// Guide Payouts JavaScript
    if (window.GuidePayoutsManager) {
        console.log('GuidePayoutsManager already exists, cleaning up...');
        if (window.guidePayoutsManager) {
            delete window.guidePayoutsManager;
        }
        delete window.GuidePayoutsManager;
    }

    // Guide Payouts Manager
    class GuidePayoutsManager {
        constructor() {
        ...
        async loadPayouts() {
            try {
                console.log('Loading guide payouts...');
                // Load completed, pending, and cancelled payouts
                const [completedResponse, pendingResponse, cancelledResponse] = await Promise.all([
                    fetch(`${this.URL_ROOT}/moderator/getCompletedGuidePayouts`),
                    fetch(`${this.URL_ROOT}/moderator/getPendingGuidePayouts`),
                    fetch(`${this.URL_ROOT}/moderator/getCancelledGuidePayouts`)
                ]);
                ...
                <tr class="account-row" data-payout-id="${payout.id}">
                    ...
                    <td class="guide-cell">
                        <div class="guide-info">
                            <div class="guide-name">${this.escapeHtml(payout.guideName)}</div>
                            <div class="guide-email">${this.escapeHtml(payout.guideEmail)}</div>
                        </div>
                    </td>
            ...
            return `
                <div class="user-details-grid">
                    ...
                    <div class="user-info-section">
                        ...
                        <div class="info-group">
                            <h4><i class="fas fa-user"></i> Guide Information</h4>
                            <div class="info-item">
                                <label>Name:</label>
                                <span>${this.escapeHtml(payout.guideName)}</span>
                            </div>
                            <div class="info-item">
                                <label>Email:</label>
                                <span>${this.escapeHtml(payout.guideEmail)}</span>
                            </div>
                            <div class="info-item">
                                <label>Phone:</label>
                                <span>${this.escapeHtml(payout.guidePhone || 'N/A')}</span>
                            </div>
                        </div>
                        <div class="info-group">
    ...
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', () => {
            window.guidePayoutsManager = new GuidePayoutsManager();
        });
    } else {
        window.guidePayoutsManager = new GuidePayoutsManager();
    }
