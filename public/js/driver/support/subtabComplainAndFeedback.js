(function () {
	...
	async function loadProblems() {
		...
		try {
			const response = await fetch(`${URL_ROOT}/Driver/getUserProblemsByUserId?filter=${encodeURIComponent(currentFilter)}`);
	...
	async function submitProblem(event) {
		...
		try {
			const response = await fetch(`${URL_ROOT}/Driver/submitProblem`, {
				method: 'POST',
				headers: {
