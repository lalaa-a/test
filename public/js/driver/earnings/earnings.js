...
(function () {
    ...
    class EarningsManager {
        ...
        async loadAll() {
            try {
                const [summaryRes, pendingRes, paidRes, refundedRes, monthlyRes] = await Promise.all([
                    fetch(`${this.URL_ROOT}/Driver/getEarningsSummary`),
                    fetch(`${this.URL_ROOT}/Driver/getEarningsByStatus/pending`),
                    fetch(`${this.URL_ROOT}/Driver/getEarningsByStatus/paid`),
                    fetch(`${this.URL_ROOT}/Driver/getEarningsByStatus/refunded`),
                    fetch(`${this.URL_ROOT}/Driver/getMonthlyEarnings`)
                ]);
        ...
        buildPendingRow(r, idx) {
            return `
                <tr>
                    ...
                    <td class="amount-cell">${this.fmt(r.driverCharge)}</td>
        ...
        buildRefundedRow(r, idx) {
            const refundAmt = r.refundAmount != null ? r.refundAmount : r.driverCharge;
            const siteStatus = parseInt(r.pDoneSite)
                ? '<span class="status-badge paid"><i class="fas fa-check"></i> Received</span>'
                : '<span class="status-badge awaiting"><i class="fas fa-hourglass-half"></i> Awaiting Payout</span>';
            return `
