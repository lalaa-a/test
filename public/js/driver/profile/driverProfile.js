...
class DriverProfileManager {
    ...
    initializeElements() {
        ...
        this.driverLicenseReview = document.getElementById('driverLicenseReview');
        this.driverLicenseEdit = document.getElementById('driverLicenseEdit');
        this.editDriverLicenseBtn = document.getElementById('editDriverLicenseBtn');
        this.cancelDriverLicenseBtn = document.getElementById('cancelDriverLicenseBtn');
        this.updateDriverLicenseBtn = document.getElementById('updateDriverLicenseBtn');
        this.licenseStatus = document.getElementById('licenseStatus');
        this.verifiedDriverLicenseNumber = document.getElementById('verifiedDriverLicenseNumber');
        this.verifiedDriverLicenseExpiry = document.getElementById('verifiedDriverLicenseExpiry');
    ...
    attachEventListeners() {
        ...
        // Driver license upload buttons
        const uploadDriverFrontBtn = document.getElementById('uploadLicenseFrontBtn');
        const uploadDriverBackBtn = document.getElementById('uploadLicenseBackBtn');
        const driverFrontInput = document.getElementById('licenseFrontInput');
        const driverBackInput = document.getElementById('licenseBackInput');
        
        if (uploadDriverFrontBtn && driverFrontInput) {
            uploadDriverFrontBtn.addEventListener('click', (e) => {
                e.preventDefault();
                e.stopPropagation();
                driverFrontInput.click();
            });
        }
        
        if (uploadDriverBackBtn && driverBackInput) {
            uploadDriverBackBtn.addEventListener('click', (e) => {
                e.preventDefault();
                e.stopPropagation();
                driverBackInput.click();
            });
        }
    ...
    async loadProfileData() {
        try {
            ...
            const response = await fetch(`${this.URL_ROOT}/Driver/retrieveBasicDriverInfo`);
    ...
    populateDisplayData(data) {
        ...
        if (this.verificationStatus) {
            const isDriverLicenseVerified = data.dLicenseStatus === true || data.dLicenseStatus === '1' || data.dLicenseStatus === 1;
            const isTouristLicenseVerified = data.tLicenseStatus === true || data.tLicenseStatus === '1' || data.tLicenseStatus === 1;
            const isVerified = isDriverLicenseVerified && isTouristLicenseVerified;

            if (isVerified) {
                this.verificationStatus.innerHTML = '<span class="status-badge status-verified"><i class="fas fa-shield-alt"></i> Verified Driver</span>';
            } else {
        ...
        // Update verified license info
        if (this.verifiedDriverLicenseNumber) {
            this.verifiedDriverLicenseNumber.textContent = data.licenseNumber || '-';
        }
        if (this.verifiedDriverLicenseExpiry) {
            this.verifiedDriverLicenseExpiry.textContent = data.licenseExpiry ? new Date(data.licenseExpiry).toLocaleDateString() : '-';
        }

        // Update driver license status badge
        if (this.licenseStatus) {
            const isDriverLicenseVerified = data.dLicenseStatus === true || data.dLicenseStatus === '1' || data.dLicenseStatus === 1;
            if (isDriverLicenseVerified) {
                this.licenseStatus.innerHTML = '<span class="status-badge status-valid">Valid</span>';
            } else {
                this.licenseStatus.innerHTML = '<span class="status-badge status-pending">Pending Verification</span>';
            }
        }

        // Update driver license photos
    ...
    initializeTouristLicenseState() {
        ...
        if (isTouristLicenseVerified) {
            ...
            // Not submitted - show submit form (like driver license)
            this.touristLicenseVerified.style.display = 'none';
            this.touristLicenseReview.style.display = 'none';
    ...
    toggleEditMode(section) {
        ...
        } else if (sectionId === 'Driver License' || sectionId === 'Tourist Driver License') {
    ...
    toggleLicenseEdit(sectionId) {
        ...
        const isDriverLicense = sectionId === 'Driver License';
        ...
        // Header status element (different id for driver vs tourist)
        const statusEl = isDriverLicense ? document.getElementById('licenseStatus') : document.getElementById('touristLicenseStatus');
    ...
    populateLicenseFormData(isDriverLicense) {
        if (isDriverLicense) {
            // Populate driver license form fields
            const licenseNumberField = document.getElementById('licenseNumber');
            const licenseExpiryField = document.getElementById('licenseExpiry');
            
            if (licenseNumberField && this.currentData && this.currentData.licenseNumber) {
                licenseNumberField.value = this.currentData.licenseNumber;
            }
            
            if (licenseExpiryField && this.currentData && this.currentData.licenseExpiry) {
                licenseExpiryField.value = this.currentData.licenseExpiry;
            }
        } else {
    ...
    populateExistingLicensePhotos(isDriverLicense) {
        if (isDriverLicense) {
            // Populate driver license photos
            if (this.currentData && this.currentData.driverLicenseFrontPhoto) {
            ...
            if (this.currentData && this.currentData.driverLicenseBackPhoto) {
    ...
    async saveProfile(section) {
        ...
        if (sectionId === 'Personal Information') {
            ...
            try {
                ...
                const response = await fetch(`${this.URL_ROOT}/Driver/editDriverPersonalInfo`, {
                    method: 'POST',
                    body: formData
                });
        ...
        // Check if this is the driver license form
        if (form.id === 'licenseForm') {
            this.handleDriverLicenseSubmission(formData);
            return;
        }
    ...
    async handleDriverLicenseSubmission(formData) {
        console.log('Updating driver license...', Object.fromEntries(formData));
        ...
        if (frontInput && frontInput.files[0]) {
            formData.append('licenseFront', frontInput.files[0]);
        }
        if (backInput && backInput.files[0]) {
            formData.append('licenseBack', backInput.files[0]);
        }
        ...
        try {
            ...
            const response = await fetch(`${this.URL_ROOT}/Driver/editDriverLicense`, {
                method: 'POST',
                body: formData
            });
            ...
            if (result.success) {
                ...
                const reviewDiv = document.getElementById('driverLicenseReview');
                const editDiv = document.getElementById('driverLicenseEdit');
                const editBtn = document.getElementById('editDriverLicenseBtn');
                const cancelBtn = document.getElementById('cancelDriverLicenseBtn');
                const saveBtn = document.getElementById('updateDriverLicenseBtn');
                ...
                const submittedDate = document.getElementById('driverLicenseSubmittedDate');
                if (submittedDate) {
    ...
    completeDriverLicenseVerification(formData) {
        ...
        const verifiedNumber = document.getElementById('verifiedDriverLicenseNumber');
        const verifiedExpiry = document.getElementById('verifiedDriverLicenseExpiry');
