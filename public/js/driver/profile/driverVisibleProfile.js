(function() {
    ...
    class DriverProfileManager {
        ...
        bindEvents() {
            ...
            const vehicleModalCloseBtn = document.querySelector('#vehicleModal .modal-close');
            ...
            if (vehicleModalCloseBtn) {
                vehicleModalCloseBtn.addEventListener('click', () => this.closeVehicleModal());
            }
        ...
        async loadProfileData() {
            try {
                ...
                if (pricingData.success) {
                    this.pricing = pricingData.pricing;
        ...
        renderProfile() {
            ...
            if (profilePhoto && data.profile_photo) {
                profilePhoto.src = `${this.UP_ROOT}${data.profile_photo}`;
            }
        ...
        renderVehicles() {
            ...
            const vehiclesHtml = this.vehicles.map(vehicle => {
                // Find pricing for this vehicle
                const vehiclePricing = this.pricing.find(p => p.vehicleId === vehicle.vehicleId);
                    ...
                    pricingHtml = `
                        <div class="vehicle-pricing">
                            <div class="pricing-item">
                                <span class="pricing-label">Per Km:</span>
                                <span class="pricing-value">$${vehiclePricing.vehicleChargePerKm.toFixed(2)}</span>
                            </div>
                            <div class="pricing-item">
                                <span class="pricing-label">Per Day:</span>
                                <span class="pricing-value">$${vehiclePricing.vehicleChargePerDay.toFixed(2)}</span>
                            </div>
                            ${vehiclePricing.minimumKm > 0 ? `<div class="pricing-item"><span class="pricing-label">Min Km:</span><span class="pricing-value">${vehiclePricing.minimumKm} km</span></div>` : ''}
                            ${vehiclePricing.minimumDays > 1 ? `<div class="pricing-item"><span class="pricing-label">Min Days:</span><span class="pricing-value">${vehiclePricing.minimumDays} days</span></div>` : ''}
                        </div>
                    `;
                } else {
                    pricingHtml = '<div class="vehicle-pricing"><p class="no-pricing">Pricing not available</p></div>';
                }

                return `
                    <div class="vehicle-card" onclick="window.driverProfileManager.showVehicleDetails(${vehicle.vehicleId})">
                        <div class="vehicle-image">
                            <img src="${this.UP_ROOT}${vehicle.frontViewPhoto || 'default-vehicle.jpg'}" alt="${vehicle.make} ${vehicle.model}">
            ...
            const reviewsHtml = this.reviews.map(review => `
                <div class="review-item">
                    <div class="review-header">
                        <div class="user-info">
                            <img src="${this.UP_ROOT}${review.travellerPhoto || '/img/signup/profile.png'}" alt="${review.travellerName}" class="avatar">
                            <div>
                                <span class="username">${this.escapeHtml(review.travellerName)}</span>
                                <span class="time">${this.formatDate(review.createdAt)}</span>
                            </div>
                        </div>
        ...
        updateRatingSummary() {
            if (!this.ratings || this.ratings.length === 0) {
                ...
                if (starsElement) {
                    starsElement.innerHTML = this.generateStars(0);
                }

                const totalElement = document.getElementById('totalReviews');
                if (totalElement) {
                    totalElement.textContent = '0 ratings';
                }
            ...
            const starsElement = document.getElementById('overallStars');
            if (starsElement) {
                starsElement.innerHTML = this.generateStars(averageRating);
            }
            ...
            const breakdownHtml = [5, 4, 3, 2, 1].map(ratingValue => {
                ...
                return `
                    <div class="bar-row">
                        ...
                        <span class="label">${ratingValue}.0</span>
        ...
        async loadReviews() {
            try {
                const response = await fetch(`${this.URL_ROOT}/RegUser/getDriverReviews/${this.driverId}`);
                const data = await response.json();
                    ...
                    <div class="vehicle-photos-grid">
                        ${photos.map(photo => `
                            <div class="vehicle-photo-item">
                                <img src="${this.UP_ROOT}/${photo.path}" alt="${photo.label}">
                            </div>
                        `).join('')}
                    </div>
                </div>
